...
import { MovieInfoApiResponse } from './types'

// We can use `typeof` here to map our dispatch types to the props, like so.
export default function MovieInfo() {
  const { id } = useParams()
  const [loading, setLoading] = useState(false)
  const [movie, setMovie] = useState<MovieInfoApiResponse | null>(null)
  const [error, setError] = useState('')

  useEffect(() => {
    if (id) {
      const idInt: number = parseInt(id, 10)
      const fetchRequest = async (idx: number) => {
        const url = `${API_ENDPOINT}/movie/${idx}?api_key=${API_KEY}`
        setLoading(true)
        try {
          const res: MovieInfoApiResponse = await callApi('get', url)
          setMovie(res)
        } catch (err) {
